...
class Nasa extends Component {
  ...
  async getId(id) {
    return fetch(
      `https://api.nasa.gov/neo/rest/v1/neo/${id}?api_key=b0ZXdqBBnafo15SUu3SVQ0eRmMDqdnIDW8TSzhE2`,
    )
      ...
      .then((Json) => {
        ...
        this.props.navigation.navigate('Details', {
          astroidDetail: this.state.astroidData,
        });
      })
      .catch((error) => {
  ...
  getAstroidId = () => {
    ...
    return fetch(
      'https://api.nasa.gov/neo/rest/v1/neo/browse?api_key=b0ZXdqBBnafo15SUu3SVQ0eRmMDqdnIDW8TSzhE2',
    )
      .then((response) => response.json())
      .then((Json) => {
        this.setState({
    ...
    return (
      <View style={styles.pageContent}>
        ...
        <Button block onPress={() => this.getAstroidId()}>
          <Text style={styles.submitText}>Random Astroid ID</Text>
        </Button>
